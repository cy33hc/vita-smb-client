...
namespace Windows {
    ...
    void ConnectionPanel()
    {
        ...
        sprintf(title, "v%s SMB %s", app_ver, lang_strings[STR_CONNECTION_SETTINGS]);
        ...
        bool is_connected = smbclient->IsConnected();
        if (is_connected)
        {
        ...
        ImVec2 size = ImGui::CalcTextSize(smb_settings->server_ip);
        if (ImGui::Button(smb_settings->server_ip, ImVec2(140, 0)))
        {
            ime_single_field = smb_settings->server_ip;
            ResetImeCallbacks();
            ime_field_size = 16;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_SERVER], smb_settings->server_ip, 15, SCE_IME_TYPE_DEFAULT, 0, 0);
            gui_mode = GUI_MODE_IME;
        }
        if (size.x > 135 && ImGui::IsItemHovered())
        {
            ImGui::BeginTooltip();
            ImGui::Text(smb_settings->server_ip);
            ImGui::EndTooltip();
        }
        ImGui::SameLine();
        ...
        size = ImGui::CalcTextSize(smb_settings->share);
        if (ImGui::Button(id, ImVec2(80, 0)))
        {
            ...
            ime_field_size = 255;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_SHARE], smb_settings->share, 255, SCE_IME_TYPE_DEFAULT, 0, 0);
            gui_mode = GUI_MODE_IME;
        }
        if (size.x > 75 && ImGui::IsItemHovered())
        {
            ImGui::BeginTooltip();
            ImGui::Text(smb_settings->share);
            ImGui::EndTooltip();
        }
        ImGui::SameLine();
        ...
        size = ImGui::CalcTextSize(smb_settings->username);
        if (ImGui::Button(id, ImVec2(80, 0)))
        {
            ...
            ime_field_size = 32;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_USERNAME], smb_settings->username, 32, SCE_IME_TYPE_DEFAULT, 0, 0);
            gui_mode = GUI_MODE_IME;
        }
        if (size.x > 75 && ImGui::IsItemHovered())
        {
            ImGui::BeginTooltip();
            ImGui::Text(smb_settings->username);
            ImGui::EndTooltip();
        }
        ImGui::SameLine();
        ...
        {
            ime_single_field = smb_settings->password;
            ResetImeCallbacks();
            ime_field_size = 24;
            ime_callback = SingleValueImeCallback;
            Dialog::initImeDialog(lang_strings[STR_PASSWORD], smb_settings->password, 24, SCE_IME_TYPE_DEFAULT, 0, 0);
            gui_mode = GUI_MODE_IME;
        }
        ImGui::SameLine();
        ...
        if (ImGui::Button(id, ImVec2(40, 0)))
        {
